...
def setup_postgresql_database(database: PostgreSQLDatabase):
    try:
        ...
        if not schema_exists:
            ...
            with open("scripts/Holiday App Database Creation Script.sql") as creation_script:
                script = creation_script.read()
                database.query_database(script)

            # Run script to populate tables
            with open("scripts/Holiday App Database Population Script.sql") as population_script:
